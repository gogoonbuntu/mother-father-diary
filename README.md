...
[![Version](https://img.shields.io/badge/Version-1.0.12-blue)](CHANGELOG.md)
...
### 🎙️ 음성 인식 입력 (Speech-to-Text)

- **원탭 토글**: 텍스트 입력 영역 하단의 마이크 FAB 버튼으로 즉시 음성 입력 시작/중지
- **다국어 자동 감지**: 기기 설정 언어에 맞춰 자동으로 인식 언어 전환 (한국어, 영어 등)
- **실시간 받아쓰기**: `ListenMode.dictation` 모드로 연속 음성을 텍스트로 변환
- **시각적 피드백**: 녹음 중 핑크색 테두리 + 부드러운 그라디언트 변환 애니메이션
- **기존 텍스트 보존**: 이미 작성한 내용 뒤에 자연스럽게 이어붙이기
...
| **음성 인식** | speech_to_text | 네이티브 STT 엔진, 다국어 지원, Dictation 모드 |
...
### 7. Speech-to-Text의 Partial Result 처리

`speech_to_text`의 `partialResults: true`는 매 콜백마다 **전체 인식 텍스트**를 반환합니다 (delta가 아님). 이를 모르고 append하면 같은 텍스트가 중복됩니다. 녹음 시작 전 텍스트를 스냅샷으로 저장하고, 매 콜백에서 `스냅샷 + 인식결과`로 **교체(replace)**하는 방식으로 해결했습니다.
...
├── diary_entry_screen.dart      # 핵심 화면: 일기 작성 + AI 변환 + 별점 + 음성 입력
...
**Made with ❤️ and ☕ by 럭키비키 일기장 Team**
